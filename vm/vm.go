...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpReturnValue:
			returnValue := vm.pop() // return value off the stack and put it on the side
			frame := vm.popFrame()  // pop the frame we just executed off the frame stack so that the next iteration of the VM’s main loop continues
...
/*
The VM:
    Reads arguments from the stack (without removing them yet).
    Executes the built-in function via the Fn field.
    Cleans up the stack (removing arguments and the function itself).
    Pushes the result (or vm.Null) onto the stack.

This design keeps the VM clean and consistent while making built-in functions work like any other function in the language.
*/
...
func (vm *VM) executeComparison(op code.Opcode) error {
	...
	default:
		return fmt.Errorf("unknown operator: %s - (%s %s)",
			code.GetOpcodeName(op), left.Type(), right.Type())
	}
}
